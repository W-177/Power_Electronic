...
int main(void)
{
 ...
 float KPi, KIi, errIsa = 0, errIsb = 0, errIsc = 0;
 ...
 for(it=0;it<LEN*STRIDE;it++)
  {
   ...
   Flr=sqrt(x[2]*x[2]+x[3]*x[3]);
   if((Flr<(8.4*Lm))&&(flg_start==0))
    {
   	 Isq=0;
   	 Isd=24.0;
    }
   else
    {
     ...
     //xin=fclipf((xin+Kis*err_wr),14.0);
   ...
   // Controladores por fase
    ...
    errIsa = errIsa + (Isa_ref - Isa);
    errIsb = errIsb + (Isb_ref - Isb);
    errIsc = errIsc + (Isc_ref - Isc);

    //saturación del integrador
    errIsa = 0.5/KIi * (errIsa > 0.5/KIi) - 0.5/KIi * (errIsa < -0.5/KIi) + errIsa * ((errIsa >= 0.5/KIi)&(errIsa <= -0.5/KIi));
    errIsb = 0.5/KIi * (errIsb > 0.5/KIi) - 0.5/KIi * (errIsb < -0.5/KIi) + errIsb * ((errIsb >= 0.5/KIi)&(errIsb <= -0.5/KIi));
    errIsc = 0.5/KIi * (errIsc > 0.5/KIi) - 0.5/KIi * (errIsc < -0.5/KIi) + errIsc * ((errIsc >= 0.5/KIi)&(errIsc <= -0.5/KIi));

/*
    Va = KPi * (Isa_ref - Isa) + KIi * errIsa;
    Vb = KPi * (Isb_ref - Isb) + KIi * errIsb;
    Vc = KPi * (Isc_ref - Isc) + KIi * errIsc;
    //Va = 1.0 * (Va > 1.0) + 0.0 * (Va < 0.0) + Va * ((Va >= 1.0)&(Va <= 0.0));
    //Vb = 1.0 * (Vb > 1.0) + 0.0 * (Vb < 0.0) + Vb * ((Vb >= 1.0)&(Vb <= 0.0));
    //Vc = 1.0 * (Vc > 1.0) + 0.0 * (Vc < 0.0) + Vc * ((Vc >= 1.0)&(Vc <= 0.0));
    Va = 0.5 * (Va > 0.5) - 0.5 * (Va < - 0.5) + Va * ((Va >= 0.5)&(Va <= -0.5));
    Vb = 0.5 * (Vb > 0.5) - 0.5 * (Vb < - 0.5) + Vb * ((Vb >= 0.5)&(Vb <= -0.5));
    Vc = 0.5 * (Vc > 0.5) - 0.5 * (Vc < - 0.5) + Vc * ((Vc >= 0.5)&(Vc <= -0.5));
   ...
   if(it%STRIDE==0)
    {
     ...
     fprintf(ptr,"%e %e %e %e %e %e %e %e %e\n",Isa,Isb,x[2],x[3],wr,Te,MIs,MFlr,x[0]);
    }
